    ...
    <div>
      <div className="question">
        ...
        <select
          id="category"
          name="category"
          onChange={handlegetQuestionByCategory}
        >
      ...
      {content.map((question) => (
        <DependentQuestion props={question} key={question.id} />
      ))}
    </div>
  );
}
...
export function DependentQuestion({ props }) {
  ...
  const handleLike = async () => {
    if (
      reactquestion.find((value) => value.username === user.email) === undefined
    ) {
  ...
  async function getReactQuestion() {
    try {
      const response = await fetch(
        `http://localhost:8000/reactquestion/${props.id}`
      );
      const json = await response.json();
  ...
  async function getQuestionLike() {
    try {
      const response = await fetch(
        `http://localhost:8000/questionlike/${props.id}`
      );
      const json = await response.json();
      setlikeNumber(json);
    } catch {
        ...
        <div className="question-content">
          ...
          <div>
            <i
              ...
              id={
                reactquestion.find((value) => value.username === user.email) ===
                undefined
                  ? "dislike-button"
                  : "like-button"
              }
    ...
    return (
      <div>
        <div className="question">
          ...
          <span>
            <NavLink to={`/`}> Trang chủ </NavLink>/
            <NavLink to={`/questions`}> Câu hỏi </NavLink>/ {match.params.id}
          </span>
        </div>
        <DependentQuestion props={QuestionCountAnswer[0]} />
        <div className="question" id="number-question">
          {QuestionBindAnswer.length} Câu trả lời
        </div>
        {QuestionBindAnswer.map((item) => (
          <Answer answer_id={item.answer_id} key={item.answer_id} />
        ))}
      </div>
    );
  } else {
...
export function APIQuestionSignIn({ match }) {
  ...
  const AddAnswer = async () => {
    ...
    //setData(data);
    window.alert(
      "Bạn đã thêm câu trả lời thành công. Hãy chờ admin duyệt câu trả lời của bạn."
    );
    await getQuestionBindAnswer(match.params.id);
  };
    ...
    return (
      <div>
        ...
        <div className="question" id="auto-height">
          <button id="leave-answer" onClick={AddAnswer}>
            Thêm câu trả lời
          </button>
        </div>
      </div>
